...
@push('setTitle') Category @endpush
          ...
          <div class="row pt-1">
            <div class="col-12 col-sm-4 col-md-2">
               <div class="d-flex align-items-center justify-content-between">
                  ...
                  <a href="javascript:void(0)" id="clear_all" class="fs-6 text-uppercase text-decoration-none text-danger fw-bold">Clear All</a>
               </div>
            </div>
               ...
               <div class="d-flex align-items-center justify-content-between">
                  ...
                  <div class="d-flex align-items-center">
                     ...
                     <select name="filter" id="filters" class="custom-select">
                        <option class="ms-2" value="latest" @if($sort === 'latest') selected @endif>Latest</option>
                        <option class="ms-2" value="discounted" @if($sort === 'discounted') selected @endif>Discounted</option>
                        <option class="ms-2" value="desc" @if($sort === 'desc') selected @endif>Descending</option>
                        <option class="ms-2" value="asc" @if($sort === 'asc') selected @endif>Ascending</option>
                        <option class="ms-2" value="low_to_high" @if($sort === 'low_to_high') selected @endif>Price: Low To High</option>
                        <option class="ms-2" value="high_to_low" @if($sort === 'high_to_low') selected @endif>Price: High To Low</option>
                     </select>
                  </div>
               </div>
            </div>
            ...
            @if ($sizes)
                @foreach ($sizes as $size)
                  <div class="mt-3 d-flex align-items-center" style="line-height: 15px">
                     <input type="checkbox" class="size" name="size_id" value="{{ $size->id }}" style="height: 18px; width:18px">
         ...
         <div class="bg-white p-2 border-top py-3">
            ...
            @if ($colors)
               ...
               <div class="mt-3 d-flex align-items-center">
                  <input type="checkbox" class="color" name="color_id" value="{{ $color->id }}" style="height:18px; width:18px">
                  <p class="mb-0 ms-2 d-flex align-items-center" style="line-height: 15px">
                     <span class="d-block border" style="width:15px;height:15px;border-radius:50%;background:{{$color->code}}"></span>
...
<script>
   ...
   document.getElementById('filters').addEventListener('change', function() {
        ...
        window.location.href = '/category?sort=' + selectedValue;
    });
